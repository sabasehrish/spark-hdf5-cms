...
object Filters {
  ...
  def filterElectronDF(spark: SparkSession, e_df: DataFrame) : DataFrame = {
    ...
    fdf = spark.sql("SELECT *, (((ABS(Electron_scEta) < 1.479) AND (((Electron_chHadIso + GREATEST(CAST(0.0 AS DOUBLE), (Electron_gammaIso + Electron_neuHadIso))) < (0.126 * Electron_pt)) OR (ABS(Electron_dEtaIn) < 0.01520) OR (ABS(Electron_dPhiIn) < 0.21600) OR (Electron_sieie < 0.01140))) OR ((ABS(Electron_scEta) >= 1.479) AND (((Electron_chHadIso + GREATEST(CAST(0.0 AS DOUBLE), (Electron_gammaIso + Electron_neuHadIso))) < (0.144 * Electron_pt)) OR (ABS(Electron_dEtaIn) < 0.01130) OR (ABS(Electron_dPhiIn) < 0.23700) OR (Electron_sieie < 0.03520)))) AS passfilter1 FROM elecFilter")
